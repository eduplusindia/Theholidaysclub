...
class User extends BaseController
{
    ...
    function addMember(){
        ...
        $this->global['pageTitle'] = 'The Holidays Clubs : Web User Add Page';
        $this->loadViews("addMember", $this->global);
    } 
    ...
    function membersList()
    {
            ...
            //$count = $this->user_model->userListingCount($searchText);
            //$returns = $this->paginationCompress ( "webusersList/", $count, 5 );
            $data['webuserRecords'] = $this->user_model->memberList();
            $this->global['pageTitle'] = 'The Holidays Clubs :Website User List';
    ...
    {   
        ...
        $this->form_validation->set_rules('Mname','Main Applicant Name','trim|required|callback_customAlpha');
        $this->form_validation->set_rules('Cname','Co-applicant Name','trim|callback_customAlpha');
        $this->form_validation->set_rules('ename','Excutive Name','trim|required|callback_customAlpha');
        $this->form_validation->set_rules('mname','Manager Name','trim|required|callback_customAlpha');
    ...
    function addNewRole()
    {
            $data['content'] = $this->user_model->getAllsesction();
            
            $this->global['pageTitle'] = 'CodeInsect :Website Add Role';
            $this->loadViews("addRoles", $this->global, $data, NULL);
    }   
    ...
    function updateRole()
    {
            ...
            $nam_arr = $this->input->post('name');
            if(count($nam_arr)>0){
            $sec_name = implode(',',$nam_arr);
            }
            $userRole = array('role'=>$rname, 'assignSections'=>$sec_name);
 ...
 /*----Gift Vouchers-----*/
    function giftVoucher()
    {
        ...
        $this->loadViews("giftVoucher",$this->global,NULL,NULL);
        }    
    }
    ...
    function addGiftVoucher()
    {
        ...
        if($this->form_validation->run() == FALSE)
         {
            ...
            $mobno =  $this->input->post('mobno');
             ...
             $giftVoucherinfo = ['name'=>$name,'email'=>$email,'mobno'=>$mobno,'address'=>$address,'voucher_code'=>$voucher_code,'date_of_generation'=>$date_of_generation,'expiration_date'=>$expiration_date,'manager_name'=>$manager_name,'venue'=>$venue];
        ...
        else
        {
           ...
           // $count = $this->user_model->userListingCount($searchText);
            ...
            $data['vouchers'] = $this->user_model->vouchers($searchText);
...
/*----End Of Gift Vouchers-----*/
   ...
   function venues($venId){
    ...
    function insertLoction(){
            ...
            $status = $this->input->post('status');
            $loc = array('locationName'=>$locationName, 'status'=>$status);
            $result = $this->user_model->addlocation($loc);
            redirect('locations');
    }   
...
/*------End 0f Venues and Locations------*/
   ...
   /*-------Configuration-----*/
     function configuration()
     {
        ...
        $data['admin_info'] = $this->user_model->admin_info(1);
